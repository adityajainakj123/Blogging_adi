...
let like = 0 ;
       await client.query(`insert into comments (commenter_user_name , description ,blog_id , support ,supporter ) values ('${comm_user_name}' ,'${description}', '${blog_id}' , ${like} , '{}') ;`)
...
router.delete ('/delete_comment' , fetchuser , async ( req,res)=>{
    const {id} = req.body ;
    try {
        const data = await client.query ( `delete from comments where comm_id = ${id} ;`) ;
        console.log(data);
        res.status(200).json({message: "comment deleted " , value :0}) ;
    } catch (error) {
        console.log(error.message);
        res.status(500).json({message:" not ok" , value :0});
    }
})
